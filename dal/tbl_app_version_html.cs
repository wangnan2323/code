...
{
    /// <summary>
    /// 数据访问类tbl_app_version_html 
    /// </summary>
    public class tbl_app_version_html : sara.dd.ldsw.idal.Itbl_app_version_html
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_app_version_html model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_htmlname != null)
            {
	            strSql1.Append("f_htmlname,");
	            strSql2.Append("'" + (model.f_htmlname) + "',");
	            strSql3.Append(":f_htmlname,");
	            parameter = new OracleParameter(":f_htmlname", OracleType.VarChar);
	            parameter.Value = model.f_htmlname;
	            parameterList.Add(parameter);
            }
            
            if (model.f_htmlversion != null)
            {
	            strSql1.Append("f_htmlversion,");
	            strSql2.Append("'" + (model.f_htmlversion) + "',");
	            strSql3.Append(":f_htmlversion,");
	            parameter = new OracleParameter(":f_htmlversion", OracleType.VarChar);
	            parameter.Value = model.f_htmlversion;
	            parameterList.Add(parameter);
            }
            
            if (model.f_htmlcontent != null && model.f_htmlcontent.Length > 0)
            {
	            strSql1.Append("f_htmlcontent,");
	            strSql2.Append("'" + (model.f_htmlcontent) + "',");
	            strSql3.Append(":f_htmlcontent,");
	            parameter = new OracleParameter(":f_htmlcontent", OracleType.Clob);
	            parameter.Value = model.f_htmlcontent;
	            parameterList.Add(parameter);
            }
            
            strSql.Append("insert into tbl_app_version_html(");
            ...
            strSql_use.Append("insert into tbl_app_version_html(");
            ...
            else
            {
                ...
                OracleParameter p = null;
			            ...
			            if (model.f_htmlcontent != null && model.f_htmlcontent.Length > 0)
	            {
	                p = new OracleParameter(":f_htmlcontent", OracleType.Clob);
	                p.Value = (model.f_htmlcontent);
	                pList.Add(p);
	                paraStrList.Add(" f_htmlcontent = :f_htmlcontent ");
	            }
	            else
	            {
	                paraStrList.Add(" f_htmlcontent = null ");
	            }
	            
	            if (paraStrList.Count > 0)
	            {
	                string updateSqlString = " update tbl_app_version_html set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_app_version_html> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_app_version_html model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_app_version_html model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_app_version_html set ");
            strSql_use.Append("update tbl_app_version_html set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_value10 != null&& columsList.Contains("f_value10"))
            {
	            ...
	            if (model.f_htmlname != null&& columsList.Contains("f_htmlname"))
            {
                strSql.Append("f_htmlname='" + (model.f_htmlname) + "',");
                strSql_use.Append("f_htmlname=:f_htmlname,");
                parameter = new OracleParameter(":f_htmlname", OracleType.VarChar);
                parameter.Value = model.f_htmlname;
                parameterList.Add(parameter);
            }
	            ...
	            if (model.f_htmlversion != null&& columsList.Contains("f_htmlversion"))
            {
	            ...
	            if (model.f_htmlcontent != null&& columsList.Contains("f_htmlcontent"))
            {
                strSql.Append("f_htmlcontent='" + (model.f_htmlcontent) + "',");
                strSql_use.Append("f_htmlcontent=:f_htmlcontent,");
                parameter = new OracleParameter(":f_htmlcontent", OracleType.Clob);
                if (model.f_htmlcontent == "")
                {
                    parameter.Value = " ";
                }
                else
            ...
            else
            {
                ...
                OracleParameter p = null;				
									...
									if (columsList.Contains("f_htmlcontent"))
				{ 
				if (model.f_htmlcontent != null && model.f_htmlcontent.Length > 0)
				{
				    p = new OracleParameter(":f_htmlcontent", OracleType.Clob);
				...
				if (paraStrList.Count > 0)
				{
					string updateSqlString = " update tbl_app_version_html set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + model.sys_id + "' ";
					if (pList.Count > 0)
					{
					   if (t == null)
					    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_app_version_html> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_app_version_html where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_app_version_html");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_app_version_html> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_app_version_html t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_app_version_html> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_app_version_html>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
...
public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_app_version_html')";
            sqlString += " where f_tablename  = 'tbl_app_version_html'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_app_version_html'";
