...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_task 
    /// </summary>
    public class tbl_task : sara.dd.ldsw.idal.Itbl_task
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_task model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            {
	            strSql1.Append("f_comment,");
	            strSql2.Append("'" + (model.f_comment) + "',");
	            strSql3.Append(":f_comment,");
	            parameter = new OracleParameter(":f_comment", OracleType.Clob);
	            parameter.Value = model.f_comment;
	            parameterList.Add(parameter);
            }
            
            strSql.Append("insert into tbl_task(");
            ...
            strSql_use.Append("insert into tbl_task(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_task set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_task> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_task model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_task model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_task set ");
            strSql_use.Append("update tbl_task set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            ...
            else
            {
				...
				if (paraStrList.Count > 0)
				{
					string updateSqlString = " update tbl_task set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + model.sys_id + "' ";
					if (pList.Count > 0)
					{
					   if (t == null)
					    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_task> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_task where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_task");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_task> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_task t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_task> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_task>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_task')";
            sqlString += " where f_tablename  = 'tbl_task'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_task'";
