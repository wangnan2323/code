...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_ld_jfbimp 
    /// </summary>
    public class tbl_ld_jfbimp : sara.dd.ldsw.idal.Itbl_ld_jfbimp
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_ld_jfbimp model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_drpcmc != null)
            {
	            strSql1.Append("f_drpcmc,");
	            strSql2.Append("'" + (model.f_drpcmc) + "',");
	            strSql3.Append(":f_drpcmc,");
	            parameter = new OracleParameter(":f_drpcmc", OracleType.VarChar);
	            parameter.Value = model.f_drpcmc;
	            parameterList.Add(parameter);
            }
            
            if (model.f_drfj != null)
            {
	            strSql1.Append("f_drfj,");
	            strSql2.Append("'" + (model.f_drfj) + "',");
	            strSql3.Append(":f_drfj,");
	            parameter = new OracleParameter(":f_drfj", OracleType.VarChar);
	            parameter.Value = model.f_drfj;
	            parameterList.Add(parameter);
            }
            ...
            {
	            strSql1.Append("f_lx,");
	            strSql2.Append("'" + (model.f_lx) + "',");
	            strSql3.Append(":f_lx,");
	            parameter = new OracleParameter(":f_lx", OracleType.VarChar);
	            parameter.Value = model.f_lx;
	            parameterList.Add(parameter);
            }
            
            if (model.f_lxid != null)
            {
	            strSql1.Append("f_lxid,");
	            strSql2.Append("'" + (model.f_lxid) + "',");
	            strSql3.Append(":f_lxid,");
	            parameter = new OracleParameter(":f_lxid", OracleType.VarChar);
	            parameter.Value = model.f_lxid;
	            parameterList.Add(parameter);
            }
            ...
            strSql.Append("insert into tbl_ld_jfbimp(");
            ...
            strSql_use.Append("insert into tbl_ld_jfbimp(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_ld_jfbimp set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_ld_jfbimp> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_ld_jfbimp model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_ld_jfbimp model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_ld_jfbimp set ");
            strSql_use.Append("update tbl_ld_jfbimp set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_drbh != null&& columsList.Contains("f_drbh"))
            {
	            ...
	            if (model.f_drpcmc != null&& columsList.Contains("f_drpcmc"))
            {
                strSql.Append("f_drpcmc='" + (model.f_drpcmc) + "',");
                strSql_use.Append("f_drpcmc=:f_drpcmc,");
                parameter = new OracleParameter(":f_drpcmc", OracleType.VarChar);
                parameter.Value = model.f_drpcmc;
                parameterList.Add(parameter);
            }
            
	            if (model.f_drfj != null&& columsList.Contains("f_drfj"))
            {
                strSql.Append("f_drfj='" + (model.f_drfj) + "',");
                strSql_use.Append("f_drfj=:f_drfj,");
                parameter = new OracleParameter(":f_drfj", OracleType.VarChar);
                parameter.Value = model.f_drfj;
                parameterList.Add(parameter);
            }
	            ...
	            if (model.f_czsj != null&& columsList.Contains("f_czsj"))
            {
	            ...
	            if (model.f_lx != null&& columsList.Contains("f_lx"))
            {
                strSql.Append("f_lx='" + (model.f_lx) + "',");
                strSql_use.Append("f_lx=:f_lx,");
                parameter = new OracleParameter(":f_lx", OracleType.VarChar);
                parameter.Value = model.f_lx;
                parameterList.Add(parameter);
            }
            
	            if (model.f_lxid != null&& columsList.Contains("f_lxid"))
            {
                strSql.Append("f_lxid='" + (model.f_lxid) + "',");
                strSql_use.Append("f_lxid=:f_lxid,");
                parameter = new OracleParameter(":f_lxid", OracleType.VarChar);
                parameter.Value = model.f_lxid;
                parameterList.Add(parameter);
            }
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_ld_jfbimp> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_ld_jfbimp where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_ld_jfbimp");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_ld_jfbimp> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_ld_jfbimp t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_ld_jfbimp> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_ld_jfbimp>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_ld_jfbimp')";
            sqlString += " where f_tablename  = 'tbl_ld_jfbimp'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_ld_jfbimp'";
