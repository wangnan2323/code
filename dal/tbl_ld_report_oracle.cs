...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_ld_report 
    /// </summary>
    public class tbl_ld_report : sara.dd.ldsw.idal.Itbl_ld_report
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_ld_report model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_bbmc != null)
            {
	            strSql1.Append("f_bbmc,");
	            strSql2.Append("'" + (model.f_bbmc) + "',");
	            strSql3.Append(":f_bbmc,");
	            parameter = new OracleParameter(":f_bbmc", OracleType.VarChar);
	            parameter.Value = model.f_bbmc;
	            parameterList.Add(parameter);
            }
            
            if (model.f_bblx != null)
            {
	            strSql1.Append("f_bblx,");
	            strSql2.Append("'" + (model.f_bblx) + "',");
	            strSql3.Append(":f_bblx,");
	            parameter = new OracleParameter(":f_bblx", OracleType.VarChar);
	            parameter.Value = model.f_bblx;
	            parameterList.Add(parameter);
            }
            
            if (model.f_bblxid != null)
            {
	            strSql1.Append("f_bblxid,");
	            strSql2.Append("'" + (model.f_bblxid) + "',");
	            strSql3.Append(":f_bblxid,");
	            parameter = new OracleParameter(":f_bblxid", OracleType.VarChar);
	            parameter.Value = model.f_bblxid;
	            parameterList.Add(parameter);
            }
            
            if (model.f_zbr != null)
            {
	            strSql1.Append("f_zbr,");
	            strSql2.Append("'" + (model.f_zbr) + "',");
	            strSql3.Append(":f_zbr,");
	            parameter = new OracleParameter(":f_zbr", OracleType.VarChar);
	            parameter.Value = model.f_zbr;
	            parameterList.Add(parameter);
            }
            
            if (model.f_zbrid != null)
            {
	            strSql1.Append("f_zbrid,");
	            strSql2.Append("'" + (model.f_zbrid) + "',");
	            strSql3.Append(":f_zbrid,");
	            parameter = new OracleParameter(":f_zbrid", OracleType.VarChar);
	            parameter.Value = model.f_zbrid;
	            parameterList.Add(parameter);
            }
            
            if (model.f_zbsj != null)
            {
	            strSql1.Append("f_zbsj,");
	            strSql2.Append("to_date('" + model.f_zbsj.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-MM-dd hh24:mi:ss'),");
	            strSql3.Append(":f_zbsj,");
	            parameter = new OracleParameter(":f_zbsj", OracleType.DateTime);
	            parameter.Value = model.f_zbsj;
	            parameterList.Add(parameter);
            }
            ...
            if (model.f_contentid != null)
            {
	            strSql1.Append("f_contentid,");
	            strSql2.Append("'" + (model.f_contentid) + "',");
	            strSql3.Append(":f_contentid,");
	            parameter = new OracleParameter(":f_contentid", OracleType.VarChar);
	            parameter.Value = model.f_contentid;
	            parameterList.Add(parameter);
            }
            
            strSql.Append("insert into tbl_ld_report(");
            ...
            strSql_use.Append("insert into tbl_ld_report(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_ld_report set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_ld_report> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_ld_report model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_ld_report model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_ld_report set ");
            strSql_use.Append("update tbl_ld_report set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_value10 != null&& columsList.Contains("f_value10"))
            {
	            ...
	            if (model.f_bbmc != null&& columsList.Contains("f_bbmc"))
            {
                strSql.Append("f_bbmc='" + (model.f_bbmc) + "',");
                strSql_use.Append("f_bbmc=:f_bbmc,");
                parameter = new OracleParameter(":f_bbmc", OracleType.VarChar);
                parameter.Value = model.f_bbmc;
                parameterList.Add(parameter);
            }
            
	            if (model.f_bblx != null&& columsList.Contains("f_bblx"))
            {
                strSql.Append("f_bblx='" + (model.f_bblx) + "',");
                strSql_use.Append("f_bblx=:f_bblx,");
                parameter = new OracleParameter(":f_bblx", OracleType.VarChar);
                parameter.Value = model.f_bblx;
                parameterList.Add(parameter);
            }
            
	            if (model.f_bblxid != null&& columsList.Contains("f_bblxid"))
            {
                strSql.Append("f_bblxid='" + (model.f_bblxid) + "',");
                strSql_use.Append("f_bblxid=:f_bblxid,");
                parameter = new OracleParameter(":f_bblxid", OracleType.VarChar);
                parameter.Value = model.f_bblxid;
                parameterList.Add(parameter);
            }
            
	            if (model.f_zbr != null&& columsList.Contains("f_zbr"))
            {
                strSql.Append("f_zbr='" + (model.f_zbr) + "',");
                strSql_use.Append("f_zbr=:f_zbr,");
                parameter = new OracleParameter(":f_zbr", OracleType.VarChar);
                parameter.Value = model.f_zbr;
                parameterList.Add(parameter);
            }
            
	            if (model.f_zbrid != null&& columsList.Contains("f_zbrid"))
            {
                strSql.Append("f_zbrid='" + (model.f_zbrid) + "',");
                strSql_use.Append("f_zbrid=:f_zbrid,");
                parameter = new OracleParameter(":f_zbrid", OracleType.VarChar);
                parameter.Value = model.f_zbrid;
                parameterList.Add(parameter);
            }
            
	            if (model.f_zbsj != null&& columsList.Contains("f_zbsj"))
            {
                strSql.Append("f_zbsj=to_date('" + model.f_zbsj.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-MM-dd hh24:mi:ss'),");
                strSql_use.Append("f_zbsj=:f_zbsj,");
                parameter = new OracleParameter(":f_zbsj", OracleType.DateTime);
                parameter.Value = model.f_zbsj;
                parameterList.Add(parameter);
            }
	            ...
	            if (model.f_content != null&& columsList.Contains("f_content"))
            {
	            ...
	            if (model.f_contentid != null&& columsList.Contains("f_contentid"))
            {
                strSql.Append("f_contentid='" + (model.f_contentid) + "',");
                strSql_use.Append("f_contentid=:f_contentid,");
                parameter = new OracleParameter(":f_contentid", OracleType.VarChar);
                parameter.Value = model.f_contentid;
                parameterList.Add(parameter);
            }
            ...
            else
            {
				...
				if (paraStrList.Count > 0)
				{
					string updateSqlString = " update tbl_ld_report set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + model.sys_id + "' ";
					if (pList.Count > 0)
					{
					   if (t == null)
					    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_ld_report> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_ld_report where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_ld_report");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_ld_report> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_ld_report t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_ld_report> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_ld_report>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_ld_report')";
            sqlString += " where f_tablename  = 'tbl_ld_report'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_ld_report'";
