...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_ld_expjhts 
    /// </summary>
    public class tbl_ld_expjhts : sara.dd.ldsw.idal.Itbl_ld_expjhts
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_ld_expjhts model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_dcpcmc != null)
            {
	            strSql1.Append("f_dcpcmc,");
	            strSql2.Append("'" + (model.f_dcpcmc) + "',");
	            strSql3.Append(":f_dcpcmc,");
	            parameter = new OracleParameter(":f_dcpcmc", OracleType.VarChar);
	            parameter.Value = model.f_dcpcmc;
	            parameterList.Add(parameter);
            }
            ...
            strSql.Append("insert into tbl_ld_expjhts(");
            ...
            strSql_use.Append("insert into tbl_ld_expjhts(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_ld_expjhts set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_ld_expjhts> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_ld_expjhts model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_ld_expjhts model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_ld_expjhts set ");
            strSql_use.Append("update tbl_ld_expjhts set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_dcrid != null&& columsList.Contains("f_dcrid"))
            {
	            ...
	            if (model.f_dcpcmc != null&& columsList.Contains("f_dcpcmc"))
            {
                strSql.Append("f_dcpcmc='" + (model.f_dcpcmc) + "',");
                strSql_use.Append("f_dcpcmc=:f_dcpcmc,");
                parameter = new OracleParameter(":f_dcpcmc", OracleType.VarChar);
                parameter.Value = model.f_dcpcmc;
                parameterList.Add(parameter);
            }
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_ld_expjhts> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_ld_expjhts where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_ld_expjhts");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_ld_expjhts> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_ld_expjhts t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_ld_expjhts> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_ld_expjhts>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_ld_expjhts')";
            sqlString += " where f_tablename  = 'tbl_ld_expjhts'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_ld_expjhts'";
