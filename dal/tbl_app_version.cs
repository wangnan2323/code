...
{
    /// <summary>
    /// 数据访问类tbl_app_version 
    /// </summary>
    public class tbl_app_version : sara.dd.ldsw.idal.Itbl_app_version
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_app_version model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_appversion != null)
            {
	            strSql1.Append("f_appversion,");
	            strSql2.Append("'" + (model.f_appversion) + "',");
	            strSql3.Append(":f_appversion,");
            ...
            if (model.f_codeversion != null)
            {
	            strSql1.Append("f_codeversion,");
	            strSql2.Append("'" + (model.f_codeversion) + "',");
	            strSql3.Append(":f_codeversion,");
	            parameter = new OracleParameter(":f_codeversion", OracleType.VarChar);
	            parameter.Value = model.f_codeversion;
	            parameterList.Add(parameter);
            }
            
            if (model.f_colorversion != null)
            {
	            strSql1.Append("f_colorversion,");
	            strSql2.Append("'" + (model.f_colorversion) + "',");
	            strSql3.Append(":f_colorversion,");
            ...
            if (model.f_apptype != null)
            {
	            strSql1.Append("f_apptype,");
	            strSql2.Append("'" + (model.f_apptype) + "',");
	            strSql3.Append(":f_apptype,");
	            parameter = new OracleParameter(":f_apptype", OracleType.VarChar);
	            parameter.Value = model.f_apptype;
	            parameterList.Add(parameter);
            }
            
            if (model.f_apptypeid != null)
            {
	            strSql1.Append("f_apptypeid,");
	            strSql2.Append("'" + (model.f_apptypeid) + "',");
	            strSql3.Append(":f_apptypeid,");
	            parameter = new OracleParameter(":f_apptypeid", OracleType.VarChar);
	            parameter.Value = model.f_apptypeid;
	            parameterList.Add(parameter);
            }
            
            if (model.f_appstatus != null)
            {
	            strSql1.Append("f_appstatus,");
	            strSql2.Append("'" + (model.f_appstatus) + "',");
	            strSql3.Append(":f_appstatus,");
	            parameter = new OracleParameter(":f_appstatus", OracleType.VarChar);
	            parameter.Value = model.f_appstatus;
	            parameterList.Add(parameter);
            }
            
            if (model.f_appstatusid != null)
            {
	            strSql1.Append("f_appstatusid,");
	            strSql2.Append("'" + (model.f_appstatusid) + "',");
	            strSql3.Append(":f_appstatusid,");
	            parameter = new OracleParameter(":f_appstatusid", OracleType.VarChar);
	            parameter.Value = model.f_appstatusid;
	            parameterList.Add(parameter);
            }
            
            strSql.Append("insert into tbl_app_version(");
            ...
            strSql_use.Append("insert into tbl_app_version(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_app_version set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_app_version> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_app_version model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_app_version> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_app_version where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_app_version");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_app_version> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_app_version t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_app_version> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_app_version>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_app_version')";
            sqlString += " where f_tablename  = 'tbl_app_version'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_app_version'";
