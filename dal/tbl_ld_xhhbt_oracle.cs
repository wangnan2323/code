...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_ld_xhhbt 
    /// </summary>
    public class tbl_ld_xhhbt : sara.dd.ldsw.idal.Itbl_ld_xhhbt
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_ld_xhhbt model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_xhbh != null)
            {
	            strSql1.Append("f_xhbh,");
	            strSql2.Append("'" + (model.f_xhbh) + "',");
	            strSql3.Append(":f_xhbh,");
	            parameter = new OracleParameter(":f_xhbh", OracleType.VarChar);
	            parameter.Value = model.f_xhbh;
	            parameterList.Add(parameter);
            }
            
            if (model.f_xhmc != null)
            {
	            strSql1.Append("f_xhmc,");
	            strSql2.Append("'" + (model.f_xhmc) + "',");
	            strSql3.Append(":f_xhmc,");
	            parameter = new OracleParameter(":f_xhmc", OracleType.VarChar);
	            parameter.Value = model.f_xhmc;
	            parameterList.Add(parameter);
            }
            ...
            strSql.Append("insert into tbl_ld_xhhbt(");
            ...
            strSql_use.Append("insert into tbl_ld_xhhbt(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_ld_xhhbt set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_ld_xhhbt> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_ld_xhhbt model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_ld_xhhbt model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_ld_xhhbt set ");
            strSql_use.Append("update tbl_ld_xhhbt set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_value10 != null&& columsList.Contains("f_value10"))
            {
	            ...
	            if (model.f_xhbh != null&& columsList.Contains("f_xhbh"))
            {
                strSql.Append("f_xhbh='" + (model.f_xhbh) + "',");
                strSql_use.Append("f_xhbh=:f_xhbh,");
                parameter = new OracleParameter(":f_xhbh", OracleType.VarChar);
                parameter.Value = model.f_xhbh;
                parameterList.Add(parameter);
            }
            
	            if (model.f_xhmc != null&& columsList.Contains("f_xhmc"))
            {
                strSql.Append("f_xhmc='" + (model.f_xhmc) + "',");
                strSql_use.Append("f_xhmc=:f_xhmc,");
                parameter = new OracleParameter(":f_xhmc", OracleType.VarChar);
                parameter.Value = model.f_xhmc;
                parameterList.Add(parameter);
            }
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_ld_xhhbt> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_ld_xhhbt where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_ld_xhhbt");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_ld_xhhbt> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_ld_xhhbt t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_ld_xhhbt> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_ld_xhhbt>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
