...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_ldbm_sbfz 
    /// </summary>
    public class tbl_ldbm_sbfz : sara.dd.ldsw.idal.Itbl_ldbm_sbfz
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_ldbm_sbfz model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_fzmc != null)
            {
	            strSql1.Append("f_fzmc,");
	            strSql2.Append("'" + (model.f_fzmc) + "',");
	            strSql3.Append(":f_fzmc,");
	            parameter = new OracleParameter(":f_fzmc", OracleType.VarChar);
	            parameter.Value = model.f_fzmc;
	            parameterList.Add(parameter);
            }
            
            if (model.f_fzbm != null)
            {
	            strSql1.Append("f_fzbm,");
	            strSql2.Append("'" + (model.f_fzbm) + "',");
	            strSql3.Append(":f_fzbm,");
	            parameter = new OracleParameter(":f_fzbm", OracleType.VarChar);
	            parameter.Value = model.f_fzbm;
	            parameterList.Add(parameter);
            }
            ...
            strSql.Append("insert into tbl_ldbm_sbfz(");
            ...
            strSql_use.Append("insert into tbl_ldbm_sbfz(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_ldbm_sbfz set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_ldbm_sbfz> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_ldbm_sbfz model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_ldbm_sbfz model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_ldbm_sbfz set ");
            strSql_use.Append("update tbl_ldbm_sbfz set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_value10 != null&& columsList.Contains("f_value10"))
            {
	            ...
	            if (model.f_fzmc != null&& columsList.Contains("f_fzmc"))
            {
                strSql.Append("f_fzmc='" + (model.f_fzmc) + "',");
                strSql_use.Append("f_fzmc=:f_fzmc,");
                parameter = new OracleParameter(":f_fzmc", OracleType.VarChar);
                parameter.Value = model.f_fzmc;
                parameterList.Add(parameter);
            }
            
	            if (model.f_fzbm != null&& columsList.Contains("f_fzbm"))
            {
                strSql.Append("f_fzbm='" + (model.f_fzbm) + "',");
                strSql_use.Append("f_fzbm=:f_fzbm,");
                parameter = new OracleParameter(":f_fzbm", OracleType.VarChar);
                parameter.Value = model.f_fzbm;
                parameterList.Add(parameter);
            }
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_ldbm_sbfz> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_ldbm_sbfz where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_ldbm_sbfz");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_ldbm_sbfz> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_ldbm_sbfz t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_ldbm_sbfz> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_ldbm_sbfz>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_ldbm_sbfz')";
            sqlString += " where f_tablename  = 'tbl_ldbm_sbfz'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_ldbm_sbfz'";
