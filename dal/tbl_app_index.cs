...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_app_index 
    /// </summary>
    public class tbl_app_index : sara.dd.ldsw.idal.Itbl_app_index
    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_app_index> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_app_index model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_app_index model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_app_index set ");
            strSql_use.Append("update tbl_app_index set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_releasedate != null&& columsList.Contains("f_releasedate"))
            {
	            ...
	            if (model.f_indextype != null&& columsList.Contains("f_indextype"))
            {
                strSql.Append("f_indextype='" + (model.f_indextype) + "',");
                strSql_use.Append("f_indextype=:f_indextype,");
                parameter = new OracleParameter(":f_indextype", OracleType.VarChar);
                parameter.Value = model.f_indextype;
                parameterList.Add(parameter);
            }
            
	            if (model.f_indextypeid != null&& columsList.Contains("f_indextypeid"))
            {
                strSql.Append("f_indextypeid='" + (model.f_indextypeid) + "',");
                strSql_use.Append("f_indextypeid=:f_indextypeid,");
                parameter = new OracleParameter(":f_indextypeid", OracleType.VarChar);
                parameter.Value = model.f_indextypeid;
                parameterList.Add(parameter);
            }
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_app_index> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_app_index where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_app_index");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_app_index> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_app_index t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_app_index> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_app_index>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
...
public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_app_index')";
            sqlString += " where f_tablename  = 'tbl_app_index'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_app_index'";
