...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_resource 
    /// </summary>
    public class tbl_resource : sara.dd.ldsw.idal.Itbl_resource
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_resource model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            strSql.Append("insert into tbl_resource(");
            ...
            strSql_use.Append("insert into tbl_resource(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_resource set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_resource> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_resource model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_resource model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_resource set ");
            strSql_use.Append("update tbl_resource set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            ...
            else
            {
				...
				if (paraStrList.Count > 0)
				{
					string updateSqlString = " update tbl_resource set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + model.sys_id + "' ";
					if (pList.Count > 0)
					{
					   if (t == null)
					    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_resource> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_resource where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_resource");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_resource> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_resource t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_resource> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_resource>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
        ...
        public string GetMaxId(Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            sqlString += " (select to_number(f_tablesys_id)+1 from tbl_num where f_tablename  = 'tbl_resource')";
            sqlString += " where f_tablename  = 'tbl_resource'";
            ...
            sqlString = "select f_tablesys_id from tbl_num where f_tablename  = 'tbl_resource'";
