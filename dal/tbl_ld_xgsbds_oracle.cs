...
namespace sara.dd.ldsw.dal
{
    ...
    /// 数据访问类tbl_ld_xgsbds 
    /// </summary>
    public class tbl_ld_xgsbds : sara.dd.ldsw.idal.Itbl_ld_xgsbds
    {
        ...
        /// <returns></returns>
        public string Add(sara.dd.ldsw.model.tbl_ld_xgsbds model, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            if (model.f_xgsbdsbh != null)
            {
	            strSql1.Append("f_xgsbdsbh,");
	            strSql2.Append("'" + (model.f_xgsbdsbh) + "',");
	            strSql3.Append(":f_xgsbdsbh,");
	            parameter = new OracleParameter(":f_xgsbdsbh", OracleType.VarChar);
	            parameter.Value = model.f_xgsbdsbh;
	            parameterList.Add(parameter);
            }
            
            if (model.f_xgsbdsmc != null)
            {
	            strSql1.Append("f_xgsbdsmc,");
	            strSql2.Append("'" + (model.f_xgsbdsmc) + "',");
	            strSql3.Append(":f_xgsbdsmc,");
	            parameter = new OracleParameter(":f_xgsbdsmc", OracleType.VarChar);
	            parameter.Value = model.f_xgsbdsmc;
	            parameterList.Add(parameter);
            }
            ...
            if (model.f_drbh != null)
            {
	            strSql1.Append("f_drbh,");
	            strSql2.Append("'" + (model.f_drbh) + "',");
	            strSql3.Append(":f_drbh,");
	            parameter = new OracleParameter(":f_drbh", OracleType.VarChar);
	            parameter.Value = model.f_drbh;
	            parameterList.Add(parameter);
            }
            
            if (model.f_fxsj != null)
            {
	            strSql1.Append("f_fxsj,");
	            strSql2.Append("to_date('" + model.f_fxsj.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-MM-dd hh24:mi:ss'),");
	            strSql3.Append(":f_fxsj,");
	            parameter = new OracleParameter(":f_fxsj", OracleType.DateTime);
	            parameter.Value = model.f_fxsj;
	            parameterList.Add(parameter);
            }
            ...
            strSql.Append("insert into tbl_ld_xgsbds(");
            ...
            strSql_use.Append("insert into tbl_ld_xgsbds(");
            ...
            else
            {
	            ...
	            {
	                string updateSqlString = " update tbl_ld_xgsbds set " + string.Join(" , ", paraStrList.ToArray()) + " where sys_id = '" + sid + "' ";
	                if (pList.Count > 0)
	                {
	                    if (t == null)
	                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string AddList(List<sara.dd.ldsw.model.tbl_ld_xgsbds> modelList, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            try
            {
               if (t == null)
                ...
                foreach (sara.dd.ldsw.model.tbl_ld_xgsbds model in modelList)
                {
                   if (t == null)
                    {
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string Update(sara.dd.ldsw.model.tbl_ld_xgsbds model, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            StringBuilder strSql_use = new StringBuilder();
            strSql.Append("update tbl_ld_xgsbds set ");
            strSql_use.Append("update tbl_ld_xgsbds set ");
            if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
	            ...
	            if (model.f_value10 != null&& columsList.Contains("f_value10"))
            {
	            ...
	            if (model.f_xgsbdsbh != null&& columsList.Contains("f_xgsbdsbh"))
            {
                strSql.Append("f_xgsbdsbh='" + (model.f_xgsbdsbh) + "',");
                strSql_use.Append("f_xgsbdsbh=:f_xgsbdsbh,");
                parameter = new OracleParameter(":f_xgsbdsbh", OracleType.VarChar);
                parameter.Value = model.f_xgsbdsbh;
                parameterList.Add(parameter);
            }
            
	            if (model.f_xgsbdsmc != null&& columsList.Contains("f_xgsbdsmc"))
            {
                strSql.Append("f_xgsbdsmc='" + (model.f_xgsbdsmc) + "',");
                strSql_use.Append("f_xgsbdsmc=:f_xgsbdsmc,");
                parameter = new OracleParameter(":f_xgsbdsmc", OracleType.VarChar);
                parameter.Value = model.f_xgsbdsmc;
                parameterList.Add(parameter);
            }
	            ...
	            if (model.f_drsj != null&& columsList.Contains("f_drsj"))
            {
	            ...
	            if (model.f_drbh != null&& columsList.Contains("f_drbh"))
            {
                strSql.Append("f_drbh='" + (model.f_drbh) + "',");
                strSql_use.Append("f_drbh=:f_drbh,");
                parameter = new OracleParameter(":f_drbh", OracleType.VarChar);
                parameter.Value = model.f_drbh;
                parameterList.Add(parameter);
            }
            
	            if (model.f_fxsj != null&& columsList.Contains("f_fxsj"))
            {
                strSql.Append("f_fxsj=to_date('" + model.f_fxsj.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-MM-dd hh24:mi:ss'),");
                strSql_use.Append("f_fxsj=:f_fxsj,");
                parameter = new OracleParameter(":f_fxsj", OracleType.DateTime);
                parameter.Value = model.f_fxsj;
                parameterList.Add(parameter);
            }
        ...
        /// <param name="clientInf"></param>
        /// <returns></returns>
        public string UpdateList(List<sara.dd.ldsw.model.tbl_ld_xgsbds> modelList, string columns, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            int column = 0;
            try
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete tbl_ld_xgsbds where  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from tbl_ld_xgsbds");
            if (whereString.Trim() != "")
            {
                strSql.Append(" where " + whereString);
            }
        ...
        /// <returns></returns>
        public List<sara.dd.ldsw.model.tbl_ld_xgsbds> GetList(string whereString, string orderByString, string columnsString, string pageSizeString, string pageIndexString, Eva.Library.Data.AccessDataTrans.IAccessDataTrans t)
        {
            StringBuilder strSql = new StringBuilder();

            strSql.Append(" select " + columnsString + " from (");
            strSql.Append(" select rownum rn,a.* from (");
            strSql.Append(" select * from tbl_ld_xgsbds t where");
            if (whereString.Trim() == "")
            {
                strSql.Append(" 1=1 ");
            }
            ...
            List<sara.dd.ldsw.model.tbl_ld_xgsbds> modelList = Eva.Library.Format.FormatEntityTool.FormatDataTableToModelList<sara.dd.ldsw.model.tbl_ld_xgsbds>(resultDataTable);
			if (Eva.Library.Configuration.ConfigurationManager.AppSettings["SqlEditMode"].ToString() == "parameter")
            {
            }
            else //如果是sql形式的话，则进行textReturn
